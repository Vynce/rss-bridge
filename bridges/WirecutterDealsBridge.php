...
class WirecutterDealsBridge extends BridgeAbstract
{
    ...
    private function json_to_html($node) {
        if ($node['type'] == 'text') {
    ...
    private function generate_content($deal) {
        ...
        foreach ($structuredContent as $node) {
            $content .= $this->json_to_html($node);
        }
    ...
    public function collectData()
    {
        ...
        foreach ($data->props->pageProps->specialEvent->eventDeals as $deal) {
            ...
            $item['content'] = $this->generate_content($deal);
